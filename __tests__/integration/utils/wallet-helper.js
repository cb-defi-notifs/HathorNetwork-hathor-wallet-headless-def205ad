...
export class WalletHelper {
  ...
  /**
   * Build a creat-token transaction proposal.
   *
   * @param params
   * @param {string} params.name Long name of the token
   * @param {string} params.symbol Token symbol
  ...
  async buildCreateToken(params) {
    ...
    const _params = { name, symbol, amount, address }
    ...
    const response = await TestUtils.request
      ...
      .send({
        ...
        change_address: _params.address,
      })
      .set({ 'x-wallet-id': this.#walletId });
  ...
  async signAndPush(params) {
    ...
    const tokenCreationBody = {
      txHex: _params.txHex,
      signatures: await TestUtils.getXSignatures(_params.txHex, _params.wallets, _params.xSignatures),
    };
